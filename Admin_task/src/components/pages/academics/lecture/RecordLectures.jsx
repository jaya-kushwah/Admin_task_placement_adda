...
const RecordLecture = () => {
  ...
  const handleConfirmDelete = () => {
    setLectures((prev) =>
      prev.filter((item) => item.id !== selectedLecture.id)
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {["Course", "Subject", "All"].map((filter) => (
            <button
              key={filter}
              className="flex items-center gap-2 bg-[#111] border border-gray-800 px-4 py-1.5 rounded-full text-xs font-medium text-gray-300 hover:border-gray-600 transition-all"
            >
              {filter} <ChevronDown size={14} className="text-gray-500" />
            </button>
          ))}

          <Link to="/academics/add_lecture">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800/40">
              {lectures.map((item) => (
                <tr
                  key={item.id}
                  className="hover:bg-white/[0.02] transition-colors group text-[13px] whitespace-nowrap"
                >
