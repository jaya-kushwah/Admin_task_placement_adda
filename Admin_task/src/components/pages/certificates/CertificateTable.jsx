...
import RevokeCertificateModel from "../../reusable/RevokeCertificateModel";
import ReissueCertificateModel from "../../reusable/ReissueCertificateModel";
...
const CertificateTable = () => {
  ...
  return (
    <div>
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {certificateData.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 md:px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-200 truncate max-w-[100px] md:max-w-none">
                          {row.name}
                        </span>
                  ...
                  <td className="hidden lg:table-cell px-6 py-4 text-[13px] text-gray-400">
                    {row.course}
                  </td>
                  <td className="hidden md:table-cell px-6 py-4 text-[13px] text-gray-400">
                    {row.date}
                  </td>
                  ...
                  <td className="px-4 md:px-6 py-4">
                    <span
                      className={`px-2 md:px-4 py-1 rounded-full text-[9px] md:text-[10px] font-bold uppercase border ${
                        row.status === "Active"
                  ...
                  <td className="px-4 md:px-6 py-4">
                    <div className="flex items-center justify-center gap-3 md:gap-4">
                      ...
                      {/* <button
                        onClick={() =>
                          row.status === "Active" && setIsModalOpen(true)
                        }
                        className={`text-[10px] md:text-[12px] font-bold underline whitespace-nowrap ${
                          row.status === "Active"
      ...
      <ReissueCertificateModel
        isOpen={activeModal === "reissue"}
        onClose={() => setActiveModal(null)}
      ></ReissueCertificateModel>
    </div>
  );
